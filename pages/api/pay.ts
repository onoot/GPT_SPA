...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const orderId = Date.now().toString(); // преобразуем в строку, чтобы избежать проблем с типами в freekassa-node

        // Создаем URL для оплаты с помощью freekassa-node
        const paymentParams = { m: merchantId, o: orderId,
            om: description, am: amount,
            curr: currency };
        const paymentUrl = await fk.api('order/create', paymentParams);
        ...
        if (user) {
            ...
            await user.update({
                ...
                paymentOrderId: orderId, // Здесь мы сохраняем идентификатор заказа для будущих уведомлений о платеже
            });
        } else {
...
// Получение уведомления о платеже от FreeKassa
export async function onPaymentReceived(req: NextApiRequest, res: NextApiResponse) {
